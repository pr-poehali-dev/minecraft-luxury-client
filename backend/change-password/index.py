...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    '''
    Business: Change user password endpoint
    Args: event - dict with httpMethod, body
          context - object with request_id
    Returns: HTTP response dict with success status or error
    '''
    ...
    cursor.execute(
        "SELECT id FROM users WHERE id = " + str(int(user_id)) + 
        " AND password_hash = '" + current_password_hash + "'"
    )
    ...
    cursor.execute(
        "UPDATE users SET password_hash = '" + new_password_hash + 
